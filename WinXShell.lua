...
is_wse = (string.find(cmd_line, '-wes') and true or false)      -- Windows Embedded Standard
...
function regist_system_property() -- handle This PC's property menu
    ...
    local path = app:info('path')
    reg_write([[HKEY_CLASSES_ROOT\CLSID\{20D04FE0-3AEA-1069-A2D8-08002B30309D}\shell\properties\command]], '', path .. '\\WinXShell.exe -ui -jcfg wxsUI\\UI_SystemInfo\\main.jcfg')
...
function regist_shortcut_ocf() -- handle shortcut's OpenContainingFolder menu
    ...
    local path = app:info('path')
    app:print(path)
    reg_write([[HKEY_CLASSES_ROOT\lnkfile\shell\OpenContainingFolderMenu_wxsStub\command]], '', path .. '\\WinXShell.exe -ocf \\\"%1\\\"')
    ...
    reg_write([[HKEY_LOCAL_MACHINE\SOFTWARE\Classes\CLSID\{B1FD8E8F-DC08-41BC-AF14-AAC87FE3073B}\InProcServer32]], '', path .. '\\wxsStub.dll')
    ...
    if ARCH == 'x64' then
      ...
      reg_write([[HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Classes\CLSID\{B1FD8E8F-DC08-41BC-AF14-AAC87FE3073B}\InProcServer32]], '', path .. '\\wxsStub32.dll')
